...
public:
	...
	bool Go(int axis, double range, double speed);  //one axis go with wait
	...
	bool DGo(int axis, double range, double speed);   //one axis go with no wait
...
private:   //NI 7334 interface	
	...
	bool Start(int axis, bool block); //start motion
...
private:   //tools function
	...
	//
	bool ChkRangeIsValid(int axis, double range);
	bool ChkHomeStatus();
