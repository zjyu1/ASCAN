...
NmcHardareRes::NmcHardareRes()
{
	...
	this->pltMotionRange[SPL_X_AXIS] = SPL_X_MOTION_RANGE - SPL_RELATIVE_X_OFFSET - SPL_RANGE_RSV - SPL_ZERO_X_OFFSET;   //260mm
	this->pltMotionRange[SPL_Y_AXIS] = SPL_Y_MOTION_RANGE - SPL_RELATIVE_Y_OFFSET - SPL_RANGE_RSV - SPL_ZERO_Y_OFFSET;   //250mm
	this->pltMotionRange[SPL_Z_AXIS] = SPL_Z_MOTION_RANGE - SPL_RELATIVE_Z_OFFSET - SPL_RANGE_RSV - SPL_ZERO_Z_OFFSET;   //220 mm
	this->pltMotionRange[SPL_R_AXIS] = SPL_R_MOTION_RANGE - SPL_RELATIVE_R_OFFSET - SPL_RANGE_RSV;
...
bool NmcHardareRes::Close()
{
	...
	//
	isOver = WaitComplete(this->maxWaitTime);
	if(isOver == false)
	{
		this->Kill();
	}
...
bool NmcHardareRes::ChkRangeIsValid(int axis, double range)
{
	//defination
	int err;
	double cur;
...
int NmcHardareRes::NI_7334Init()
{
	...
	err =   flex_select_signal(brdId,NIMC_RTSI2,NIMC_PHASE_B1);	 //X axis
	err |=   flex_select_signal(brdId,NIMC_RTSI3,NIMC_PHASE_A1);

	err |=   flex_select_signal(brdId,NIMC_RTSI4,NIMC_PHASE_B2);   //Y axis
	err |=   flex_select_signal(brdId,NIMC_RTSI5,NIMC_PHASE_A2);

	err |=   flex_select_signal(brdId,NIMC_RTSI6,NIMC_PHASE_B3);   //Z axis
	err |=   flex_select_signal(brdId,NIMC_RTSI7,NIMC_PHASE_A3);

	err |=   flex_select_signal(brdId,NIMC_RTSI0,NIMC_RTSI_SOFTWARE_PORT); //start

	err |=   flex_select_signal(brdId,NIMC_RTSI1,NIMC_RTSI_SOFTWARE_PORT); //stop 
	if(err != SPL_OK)
	{
		ErrPrintf("flex_select_signal() or flex_set_port() error, pls chk!");
		return err;
	}



	mustOn = 0xFF; //start is high
	mustOff = 0; //stop is high
	
	err = flex_set_port(brdId,NIMC_RTSI_PORT,mustOn,mustOff,0xFF); 

	mustOn = 0; //start is high
	mustOff = 0x02; 
	err = flex_set_port(brdId,NIMC_RTSI_PORT,mustOn,mustOff,0xFF); 
...
//start move one axis
bool NmcHardareRes::Start(int axis, bool block)
{
	int status = 0;
	bool isOver = false;
...
bool NmcHardareRes::GoZero()
{
	...
	//find home
	err = flex_find_reference(this->boardId, NIMC_AXIS_CTRL, SPL_3AXIS_VECTOR_SPACE, NIMC_FIND_HOME_REFERENCE);
	if(err != SPL_OK)
	{
		ErrPrintf("fatal error, find home error,pls chk!");
		return false;
	}

	//wait complete
	isOk = this->NI_WaitZero(this->maxWaitTime);
	if(isOk != true)
	{		
		ErrPrintf("fatal error, wait home error,pls chk!");
		return false;
	}

	//find index reference
	err = flex_find_reference(this->boardId, NIMC_AXIS_CTRL, SPL_3AXIS_VECTOR_SPACE, NIMC_FIND_INDEX_REFERENCE);
	if(err != SPL_OK)
	{
		ErrPrintf("fatal error, find index error,pls chk!");
		return false;
	}
	...
	for(int i = SPL_X_AXIS; i<= SPL_R_AXIS;i++)
	{
		err = flex_reset_pos(this->boardId,(u8)i,0,0,SPL_DIRECT);
		if(err != SPL_OK)
		{
			ErrPrintf("fatal error, reset axis pos, pls chk!");
			return false;
		}
		this->curPos[i] = 0;
		this->curVeloc[i] = 0;
	...
	//go to relative zero point
	for(int i = SPL_X_AXIS; i< SPL_R_AXIS;i++)
	{
		isOk = Go(i, this->relativeZeroPt[i], SPL_DFT_VELOC);
		if(isOk != true)
		{
			ErrPrintf("fatal error, goto relative zero point, pls chk!");
			return false;
		}

		isOk = this->GetCurPos(i,&pos);
		if(isOk != true || abs(pos) > SPL_LOC_TOLERANCE )
		{
			ErrPrintf("fatal error, get relative zero point, pls chk!");
			return false;
		}
	}
...
bool NmcHardareRes::DGo(int axis, double range, double speed)
{
	//defination
	bool isOk;
	double time;
...
bool NmcHardareRes::SetGo(int axis, double range, double speed)
{
	...
	if(err != SPL_OK)
	{
		ErrPrintf("motion para loading failor, pls chk!");
		return false;
	}
	...
	if(this->ChkVelocIsValid(axis, speed) != true)
	{
		ErrPrintf("speed is exceed the max or min, pls chk!");
		return false;
	}
